...
This example demonstrates the use of the Wi-Fi module with the Pitaya Go board to retrieve time information from the time server.
...
You can find the source code and the project file of the example in the following folder: [pitaya-go/examples/wifi/time_client_example](https://github.com/makerdiary/pitaya-go/tree/master/examples/wifi/time_client_example).

Open terminal and navigate to the directory with the example Makefile:

``` sh
cd ./pitaya-go/examples/wifi/time_client_example/armgcc
```
...
If compiled successfully, the firmware is located in `time_client_example/armgcc/_build` with the name `nrf52840_xxaa.hex`.
...
The application is now programmed and running. Perform the following steps to retrieve time information from the time server:
...
6. Use command `wifi sntp *.pool.ntp.org` to configure what NTP server the SNTP client should use.

7. Use command `systime` to retrieve the system time from the NTP server. Observe that the time will display in the console after a while.

	![](assets/images/time-client-example-terminal.png)

8. You can also use the [Web Device CLI](https://wiki.makerdiary.com/web-device-cli/) application to test this example.
...
[![](assets/images/time-client-example-web.png)](assets/images/time-client-example-web.png)
